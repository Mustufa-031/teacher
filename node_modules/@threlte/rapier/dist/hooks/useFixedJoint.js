...
export const useFixedJoint = (anchorA, frameA, anchorB, frameB) => {
    ...
    const jfA = new Quaternion().setFromEuler(isInstanceOf(frameA, 'Euler') ? frameA : euler.fromArray(frameA));
    const jaB = isInstanceOf(anchorB, 'Vector3') ? anchorB : new Vector3().fromArray(anchorB);
    const jfB = new Quaternion().setFromEuler(isInstanceOf(frameB, 'Euler') ? frameB : euler.fromArray(frameB));
